...
This is a mini update on top of 0.0.1.4. The 404 updater bug is already fixed.
You still build the Windows installer on **your PC** (this environment cannot produce a signed `.exe`).
...
2. Paste everything in `supabase/quota.sql` (also on the `hive` website repo)
3. Click **Run**
...
## 4. Publish the download (public, no GitHub login)

1. Open https://github.com/samkomedved319-dev/hive-releases/releases/new
2. Tag: `v0.0.1.5`
3. Title: `Hive 0.0.1.5`
4. Attach `Hive-Setup-0.0.1.5.exe`
5. Publish release
6. Tick this release as **latest**

## 5. Point the in-app updater at the new file

Edit `latest.json` on **HiveSOURCE** `main` to:

```json
{
  "version": "0.0.1.5",
  "downloadUrl": "https://github.com/samkomedved319-dev/hive-releases/releases/download/v0.0.1.5/Hive-Setup-0.0.1.5.exe",
  "notes": [
...
Copy the same JSON into `Hive-Desktop/latest.json`.
...
People already on 0.0.1.3 / 0.0.1.4 click **Update and restart** in Hive. That downloads 0.0.1.5 from hive-releases.
